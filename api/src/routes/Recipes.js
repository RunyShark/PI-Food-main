...
router.get("/", async (req, res, next) => {
  ...
  try {
    if (name) {
      apiExter = (
        await axios.get(
          `https://api.spoonacular.com/recipes/complexSearch?apiKey=59c03d4a2b7d4aedb02725fd21e92176&number=100&addRecipeInformation=true`
        )
      ).data.results
...
router.get("/:id", async (req, res, next) => {
  try {
    ...
    if (id.includes("-")) {
      ...
      esta = (
        await axios.get(
          `https://api.spoonacular.com/recipes/${id}/information?apiKey=59c03d4a2b7d4aedb02725fd21e92176`
        )
      ).data;
